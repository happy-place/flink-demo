...
public class Producer implements Runnable, AutoCloseable {
  ...
  public void run() {
    ...
    while (isRunning) {
      ...
      int partition = (int)(transaction.accountId % size);

      // 注：不指定分区的话可能出现倾斜，导致flink sql 有的分区有数据，有的分区没数据，窗口无法关闭
      ProducerRecord<Long, Transaction> record =
          new ProducerRecord<>(topic, partition, millis, transaction.accountId, transaction);
