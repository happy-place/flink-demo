...
package com.bigdata.flink.proj.testing.taxi;

import com.bigdata.flink.proj.taxi.app.HourlyTipsExercise;
import com.bigdata.flink.proj.taxi.app.HourlyTipsSolution;
import com.bigdata.flink.proj.taxi.app.HourlyTipsSolution2;
import com.bigdata.flink.proj.testing.base.TaxiRideTestBase;
import com.bigdata.flink.proj.testing.base.TestFareSource;
import com.bigdata.flink.proj.testing.base.TestSink;
import com.bigdata.flink.proj.testing.base.Testable;
...
public class HourlyTipsTest extends TaxiRideTestBase<Tuple3<Long, Long, Float>> {
	...
	public void testOneDriverOneTip() throws Exception {
		...
		Tuple3<Long, Long, Float> max = Tuple3.apply(t(60).toEpochMilli(), 1L, 1.0F);
	...
	public void testTipsAreSummedByHour() throws Exception {
		...
		Tuple3<Long, Long, Float> hour1 = Tuple3.apply(t(60).toEpochMilli(), 1L, 6.0F);
		Tuple3<Long, Long, Float> hour2 = Tuple3.apply(t(120).toEpochMilli(), 1L, 10.0F);
	...
	public void testMaxAcrossDrivers() throws Exception {
		...
		Tuple3<Long, Long, Float> hour1 = Tuple3.apply(t(60).toEpochMilli(), 1L, 6.0F);
		Tuple3<Long, Long, Float> hour2 = Tuple3.apply(t(120).toEpochMilli(), 2L, 20.0F);
		...
		// 顺序需要时一致的
		 assertEquals(expected, results(source));
		// 顺序可以不一致
//		assertThat("Join results don't match", results(source), containsInAnyOrder(expected.toArray()));
	}
...
//		Testable javaSolution = () -> HourlyTipsSolution.main(new String[]{});
		Testable javaSolution = () -> HourlyTipsSolution2.main(new String[]{});
